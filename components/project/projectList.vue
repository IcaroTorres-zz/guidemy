    ...
    <v-list two-line dense subheader>
      <template v-for="(project, pidx) in listedProjects">
        <v-subheader inset :key="`pidx-${project.id}`" style="max-height: 28px; line-height: 28px; margin-bottom: -15px;">
          <nuxt-link :class="{'info--text': project.status === 0, 'warning--text': project.status === 1, 'body-2': true}" :to="`/project/${project.id}/activities`">{{project.title}}</nuxt-link>
          <v-spacer/>
          <div class="caption">{{ project.created | locale }}</div>
        </v-subheader>
        <v-list-tile  :key="project.id" class="pa-0">
          <v-list-tile-action>
            <v-tooltip top>
