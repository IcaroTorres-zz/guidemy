<template>
  <v-expansion-panel>
    <v-expansion-panel-content :class="`elevation-0 ${color}`" hide-actions expand>
      <v-flex xs12 slot="header" class="px-0">
        <v-layout align-center light class="transparent">
          <v-icon color="secondary">drag_indicator</v-icon>
          <span slot="activator">{{task.title}}</span>
          <v-spacer/>
          <v-btn small icon flat class="ma-0" color="error" v-if="canRemove">
            <v-icon small>delete</v-icon>
          </v-btn>
          <v-btn small icon flat class="ma-0" color="light-green accent-2" v-if="task.status !== 1">
